  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-md">
      <form class="space-y-5" @submit.prevent="submit">
        <DialogHeader>
          <DialogTitle>重命名表情</DialogTitle>
          <DialogDescription>名称用于在表情资产中识别和查找这组图片。</DialogDescription>
        </DialogHeader>

        <div class="space-y-2">
          <div class="flex items-center justify-between gap-3">
            <Label for="rename-expression-name">表情名称</Label>
            <span class="text-xs tabular-nums text-muted-foreground">{{ name.length }} / 80</span>
          </div>
          <Input
            id="rename-expression-name"
            v-model="name"
            maxlength="80"
