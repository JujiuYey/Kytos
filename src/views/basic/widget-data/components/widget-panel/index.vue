...
import { structSections } from '@/views/basic/widget-data/mock-data/sections';
import type { StructSection } from '@/views/basic/widget-data/mock-data/sections';
...
const props = defineProps<{
   ...
   * 桥梁类型列表
   */
  bridgeTypes: Array<{ id: string; name: string }>;
  /**
   * 构件列表
   */
  widgetList: Widget[];
...
const emit = defineEmits<{
  (e: 'viewAssessmentStandards', section: StructSection): void;
}>();

// 过滤出指定类型的构件
function getWidgetsByType(type: WidgetType) {
  return props.widgetList
    .filter(widget => widget.type === type)
    .sort((a, b) => a.sequence - b.sequence);
}

function handleAssessmentStandards(section: StructSection) {
  emit('viewAssessmentStandards', section);
}
</script>
  ...
  <div class="flex-1 p-6 overflow-auto">
    <div class="mb-6 ">
      <h2 class=" mb-6 text-2xl font-bold tracking-tight">
        {{ bridgeTypes.find(t => t.id === selectedType)?.name || '请选择桥梁类型' }}
      </h2>
    </div>
      ...
      <template v-for="section of structSections" :key="section.key">
        <div class="space-y-4">
          <h3 class="flex items-center justify-between">
            ...
            <Button :disabled="!selectedType" @click="handleAssessmentStandards(section)">
